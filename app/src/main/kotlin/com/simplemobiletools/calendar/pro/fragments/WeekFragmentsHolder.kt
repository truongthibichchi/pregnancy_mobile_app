...
class WeekFragmentsHolder : MyFragmentHolder(), WeekFragmentListener {
            ...
            addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    if (isGoToTodayVisible != shouldGoToTodayBeVisible) {
                        (activity as? CalendarMainActivity)?.toggleGoToTodayVisibility(shouldGoToTodayBeVisible)
    ...
    private fun setupWeeklyActionbarTitle(timestamp: Long) {
        ...
        if (startDateTime.monthOfYear == endDateTime.monthOfYear) {
            ...
            (activity as CalendarMainActivity).updateActionBarTitle(newTitle)
        } else {
            val endMonthName = Formatter.getMonthName(context!!, endDateTime.monthOfYear)
            (activity as CalendarMainActivity).updateActionBarTitle("$startMonthName - $endMonthName")
        }
        (activity as CalendarMainActivity).updateActionBarSubtitle("${getString(R.string.week)} ${startDateTime.plusDays(3).weekOfWeekyear}")
    }
